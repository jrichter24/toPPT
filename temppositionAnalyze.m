...
for kk=1:numel(myText)
    ...
    for ii=1:numel(FontSize)
        ...
        legendCertainChildPostition  = get(legendchilds(6),'Position');
    ...
    curTester = AxesPos3.*Pos1;
    testerMean(kk) = mean(curTester);
    testerVar(kk) = var(curTester);
...
figureTester = figure;
plot(1:numel(myText),testerMean)

figureTesterVar = figure;
plot(1:numel(myText),testerVar)