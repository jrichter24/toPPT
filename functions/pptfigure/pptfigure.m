...
%% by Jens
%reconstruct original varargin
myArg = varargin{2};
countMyArgEP = 0;

if strcmp(myArg.externalParameters,'')
    countMyArgEP = length(myArg.externalParameters);
end

%copyVarargin = cell(1+countMyArgEP);

copyVarargin{1} = varargin{1};

if countMyArgEP>1
    for ii=2:countMyArgEP
        copyVarargin{2} = myArg.externalParameters{ii-1};
    end
end


varargin = copyVarargin;


%% Read all property values
prop.autoGroupMin = myArg.defaultAutoGroupMin;
prop.slideNumber  = myArg.defaultSlideNumber;
prop.top          = myArg.userTop;
prop.width        = myArg.userWidth;
prop.height       = myArg.userHeight;
prop.left         = myArg.userLeft;
...
if isempty(varargin) || ~isscalar(varargin{1}) || ~ishandle(varargin{1})
    ...
    if ~isempty(varargin) && strcmpi(varargin{1},'add')
        ...
        try
            num2strH = num2str(H);
        catch % Matlab 2014b fix
            num2strH =  num2str(H.Number);
        end
...
if ~strcmpi(get(H,'type'),'figure')
    error(['Invalid input to pptfigure: handle ' num2strH ...
        ' is not a figure']);
end
...
if strcmp(version('-release'),'2014b')
    ...
    for c = 1:length(axesListTemp)
        [az el] = view(axesListTemp(c));
        if az~=0 || el~=90
    ...
    % Find all legends
    legends = findall(H,'type','legend');
    legendObjects = legends;
    
    %Get String from legend and place it into axis - We try to copy as much
    %information as possible from legend into dummy axis
    if ~isempty(legends)
        dummyLegendAxes = axes('Parent',H);
...
%     calledIndex = 0;
    ...
    for ii=1:numel(legends)
        ...
        for jj=1:numel(legends(ii).String)
            ...
            numElements = numel(legends(ii).String);
            
            heightPerElementPercent = 1/numElements;
            
            %yPos = curLegendPos(4)/numel(legends(ii).String)*(numel(legends(ii).String)-jj+1);
            
            %yPos = heightPerElementPercent/2+(jj-1)*heightPerElementPercent;
            
            yPos = heightPerElementPercent/2+(numel(legends(ii).String)-jj)*heightPerElementPercent;
            ...
            %set(curText,'Position',[curLegendPos(1),yPos,0]);
...
%             calledIndex = calledIndex+1;
            ...
            % same displayName as the legend string
        ...
        %         curLegendText = legends(ii).String;
%         curLegendPos = legends(ii).Position;
%         dummyLegendAxes.Position = legends(ii).Position;
%         %curAddAxis = axesList(1);
%         text(curLegendPos(1),curLegendPos(2),curLegendText,'Parent',dummyLegendAxes);
%         %dummyLegendAxes.
...
%     % In 2014b we get the legend as child from H
%     if strcmp(version('-release'),'2014b')
%         objects = [objects; findall(H,'type','legend')];
%     end
end
...
% This a hack for Matlab 2014b because num2str("fiugre") is generating an
% error
...
print('-dmeta',['-f' num2strH]) %#ok<MCPRT>

%pic = invoke(slide.Shapes,'PasteSpecial',3);

% Hack
picShapeRange = invoke(slide.Shapes,'Paste'); % or: picShapeRange = invoke(slide.Shapes,'PasteSpecial', 3); (as required)
...
myArg.objectHeight         = round(metaHeight);
myArg.objectWidth          = round(metaWidth);
%%% Calculate position parameters%%%


postioningParameters = getPosParameters(myArg);

prop.width  = round(postioningParameters.width/72);
prop.height = round(postioningParameters.height/72);
prop.top    = round(postioningParameters.top/72);
prop.left   = round(postioningParameters.left/72);
...
%                 set(objects(obj),'visible','on');
%             else
                set(get(objects(obj),'children'),'visible','on'); % HINT This causes the legend to becomse visible (not the outer boy)
%             end
%         
        end
        
        %%%Add on from http://www.mathworks.com/matlabcentral/fileexchange/30124-smart-powerpoint-exporter
       
        
%         colorbarfound = 1;
%         colorbarfound2 = 1;
%         
%         mycolorbar = findobj(get(H,'Children'),'Tag','Colorbar');
%         display(num2str(mycolorbar));
%         display(num2str(objects(obj)));
%         
...
%         end
%         
%         if objects(obj) == mycolorbar
%             display('HEre I am');
%             colorbarfound2 = 0;
%         end
        
        %if colorbarfound == 1 && colorbarfound2 == 1
        if strcmp(get(objects(obj),'Tag'),'TMW_COLORBAR')
            display('Colorbar-Tag');
...
%          end
        ...
        if ~(strcmp(type,'text') && isempty(deblank(txtString))) && ...
            ...
            % Hack
            picShapeRange = invoke(slide.Shapes,'Paste'); % or: picShapeRange = invoke(slide.Shapes,'PasteSpecial', 3); (as required)
            pic = invoke(picShapeRange,'Item',1); % HINT HERE WE ACUTALLY INVOKING EVERYTTHING
